...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> sendUssdRequest() async {
    ...
    try {
      ...
      responseMessage = await UssdService.makeRequest(
          simData.cards.first.subscriptionId, _requestCode,  Duration(seconds: 60), // timeout (optional) - default is 10 seconds
      );
      setState(() {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          title: const Text('Ussd Plugin demo by me'),
        ),
        body: Container(
              ...
              children: <Widget>[
                TextFormField(
                  decoration: const InputDecoration(
                    labelText: 'Enter your code please Code',
                  ),
                  onChanged: (newValue) {
                    setState(() {
